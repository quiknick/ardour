...
Surface::redisplay (PBD::microseconds_t now, bool force)
{
	...
	/* Icon V1-M color update: full RGB SysEx for all 8 strips */
	if ( is_v1m ) {
		...
		write (display_colors_on_v1m(rgb_bytes));
	}

	/* Existing code: timecode, jog ring, etc. ... */
	for (Strips::iterator s = strips.begin(); s != strips.end(); ++s) {
...
/** display color_values on the 8 scribble strips of the Icon V1-M **/
MidiByteArray
Surface::display_colors_on_v1m (const uint8_t rgb_values[24]) const
{
	/* Icon V1-M/P1-M color SysEx: F0 00 02 4E 16 14 [8×(R G B)] F7
